...
function getModel() {
    ...
    model = tf.sequential();
    model.add(tf.layers.conv2d({inputShape:[28, 28, 1],padding : "same",kernelSize : [3,3], filters : 16,activation : "relu"}));
    model.add(tf.layers.maxPooling2d({poolSize:[3,3]}));
    model.add(tf.layers.conv2d({filters : 16,activation : "relu",kernelSize:[3,3]}));
